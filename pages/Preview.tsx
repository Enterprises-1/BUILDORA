...
import { 
  ...
  AlertTriangle, X, DollarSign, User, Mail, Send, CheckCircle2,
  ShieldAlert, RefreshCw, Monitor, Zap
} from 'lucide-react';
...
const Preview: React.FC = () => {
  ...
  // Enterprise Demo Logic
  const [isIframeBlocked, setIsIframeBlocked] = useState(false);
  const [iframeLoaded, setIframeLoaded] = useState(false);
  const [showFallbackMessage, setShowFallbackMessage] = useState(false);
  const iframeRef = useRef<HTMLIFrameElement>(null);
  // Fix: Replace NodeJS.Timeout with ReturnType<typeof setTimeout> to solve 'Namespace global.NodeJS has no exported member Timeout'
  ...
  useEffect(() => {
    const fetchAsset = async () => {
      ...
      if (found) {
        ...
        // Handle direct redirect mode immediately
  ...
  useEffect(() => {
    // Start heuristic monitor if we are in iframe mode
    if (product && product.previewMode === 'iframe' && !iframeLoaded) {
      timeoutRef.current = setTimeout(() => {
        if (!iframeLoaded) {
          console.warn("Iframe heuristic check: Load timed out or blocked by security headers.");
          setIsIframeBlocked(true);
          handleFallbackRedirect(product.demoUrl, true);
        }
      }, 5000); // 5 second enterprise timeout
    }
    return () => {
  ...
  const handleFallbackRedirect = (url: string, isAutomatic = false) => {
    if (isAutomatic) {
      ...
      // Brief delay to allow the user to read the message before the new tab opens
      setTimeout(() => {
  ...
  if (!product) return (
    <div className="min-h-screen bg-background flex flex-col items-center justify-center p-6 text-center">
      <h2 className="text-2xl font-black text-white mb-4 tracking-tighter uppercase">Infrastructure Node Missing</h2>
  ...
  return (
    <div className="min-h-screen bg-white relative flex flex-col">
          ...
          {showToolbar && (
            <div className="bg-black/95 backdrop-blur-xl border border-white/10 rounded-2xl p-2 flex items-center gap-6 shadow-2xl animate-fade-in ring-1 ring-white/5">
              <div className="flex flex-col px-4 py-1">
                <span className="text-[7px] font-black uppercase text-gray-500 tracking-[0.3em]">Cloud Asset</span>
              ...
              <div className="flex items-center gap-2 pr-1">
                ...
                <button 
                  ...
                  title="Launch Direct Tab"
                >
                  <ExternalLink size={16} />
                </button>
                <Link to="/available-websites" className="text-gray-500 hover:text-white p-3 transition-colors" title="Terminate Preview">
                  <X size={16} />
                </Link>
              </div>
            </div>
      ...
      {/* Enterprise Demo Iframe Container */}
      <div className="flex-1 w-full bg-white relative overflow-hidden flex flex-col items-center justify-center">
        {showFallbackMessage && (
          <div className="absolute inset-0 z-50 bg-background flex flex-col items-center justify-center text-center p-8 animate-fade-in">
             ...
             <h3 className="text-3xl font-black text-white mb-4 tracking-tighter uppercase">Direct Redirect Active</h3>
             <p className="text-gray-400 text-lg font-medium max-w-md leading-relaxed">
               This demo build requires native browser context. Opening in a secure secondary tab for full functionality.
             </p>
             <button 
               onClick={() => handleFallbackRedirect(product.demoUrl)}
        ...
        {isIframeBlocked && !iframeLoaded && !showFallbackMessage && (
          <div className="absolute inset-0 z-40 bg-background/95 backdrop-blur-md flex flex-col items-center justify-center text-center p-8 animate-fade-in">
             ...
             <h3 className="text-2xl font-black text-white mb-4 tracking-tighter uppercase">Security Protocol Fallback</h3>
             <p className="text-gray-400 text-sm font-medium max-w-sm mb-10">
               Standard iframe preview is restricted by target enterprise headers. Transitioning to direct audit link.
             </p>
             <button 
               onClick={() => handleFallbackRedirect(product.demoUrl)}
             ...
             >
               Launch Audit Tab <ExternalLink size={14} />
             </button>
          </div>
        )}
        ...
        {product.previewMode === 'iframe' && (
          <iframe 
            ...
            title={`Enterprise Demo: ${product.name}`}
            loading="lazy"
          />
        )}
      ...
      {/* Acquisition Intelligence Overlay */}
      {isOfferModalOpen && (
          ...
          <div className="bg-background-surface border border-border-light w-full max-w-lg rounded-[3rem] overflow-hidden shadow-2xl relative ring-1 ring-white/5">
            ...
            {showSuccess ? (
              <div className="p-16 text-center space-y-6">
                ...
                <p className="text-gray-400 text-lg font-medium">Your acquisition proposal has been logged to the enterprise ledger. Our team will verify credentials and respond.</p>
              </div>
            ) : (
              <form onSubmit={handleBidSubmit} className="p-12 space-y-8">
                ...
                >
                  {isSubmitting ? 'PROCESSING LEDGER...' : <>TRANSMIT OFFER <Send size={18} /></>}
                </button>
              </form>
            )}
          </div>
